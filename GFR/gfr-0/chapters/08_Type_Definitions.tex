...
\section{Variant Types \& Algebraic Data Types}
...
Variant types are similar to a simplified form for a series of case classes (\sref{sec:case-classes}) and case objects (\sref{sec:case-objects}), with shared interface and a common ancestor. Variant types are also an implementation of {\em algebraic data types}, and case classes and case object may be used to achieve similar properties as well. Pattern matching is possible on variant types, as it would be done on case classes and case objects. 
...
The form ~\lstinline!$\id$! is called {\em constant variant constructors}. The objects that represent constant variant constructors are singletons. 
