...
\section{Self, This, Super, Outer \& Module}
...
Unlike languages like Java, Scala, C\# and similar, Gear does not have dedicated language construct for creating new instances of classes. Instead, all such attempts are made through the ~\lstinline!Class#new!~ method (not to be confused with ``static'' ~\lstinline@Class.new@, which is for dynamic creation of ad-hoc classes), which passes all the arguments to the appropriate constructor (and is defined using \code{Variadic_Arguments}). 

It is indeed possible to write custom ~\lstinline!$T$#new! implementations, where $T$ is the class---or even trait!---for which the custom implementation is, and that implementation may decide to use ~\lstinline!Class#new!~ or not (typical use case: a factory method without the usual corporate factory names).
...
A simple instance creation expression is of the form ~\lstinline!new $c$!, where $c$ is a constructor invocation (\sref{sec:constructor-invocations}). Let $T$ be the type of $c$, then $T$ must denote a type instance of a non-abstract subtype of \code{Any}. $T$ must also conform to the {\em self type} of the class denoted by $T$. The expression is evaluated by invoking ~\lstinline!Class#new!, where the method is invoked with \code{self} being $T$, thus skipping any custom overrides of the method. Note that $c$ might be an instantiated type variable, in which case it might not be entirely possible to guarantee that the expression will be evaluated successfully -- no matching and accessible constructor might be available in the instantiated type. 
...
Block_Expr2     ::= 'do' [Block_Args] Block 'end'
...
\begin{enumerate}
  ...
  \item When no block argument was given and the invocation happens inside a sequence comprehension, a way to generate next value in the sequence. The value of the expression is then the value ``\code{()}'' of type \code{Unit}. % TBD: this needs further description as to how the value is propagated to the sequence - maybe via a paused fiber, or maybe simply throwing? 
...
\begin{itemize}
  ...
  \item \code{Loop}, for generating sequence values. To test if a yield to a sequence is available, use \code{Yield.loop_available?}, defined in Gear's \code{Language} module. 
...
\subsubsection{Slice Expression}
...
\section{Sequence Comprehensions}
...
% seq idea
% {| expr1 |}
% begin expr1 catch Loop(x) x; otherwise Seq.empty end
...
\section{Literal Conversions \& Collection Builders}
...
Template_Stat  ::= Use
                 ...
                 | 'prepend' Stable_Id [Cond_Modifier]
