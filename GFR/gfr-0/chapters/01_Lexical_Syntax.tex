...
% constant: http://dlang.org/const3.html
% +immutable, +immutable/transitive, +mutable
% freezes self; freezes self and everything contained and not mutable; nothing
% +attribute
\begin{lstlisting}
abstract      advice        advice-execution            after
alias         annotation    arguments     around        as
aspect        assert        atomic        attribute     begin
before        bitfield      break         broken        case
catch         class         clone         cloned        constant
constraint    constructor   declare       def           delta
destructor    digits        dispatch      do            else
elsif         end           ensure        enum          execution
exhausted     extends       final         for           for-some
foreign       function      get           goto          handler
...
\begin{enumerate}
  \item The tokens preceding the newline can form a syntactically complete expression.
\end{enumerate}

Since Gear may be interpreted in a REPL\footnote{Read-Eval-Print Loop} fashion, there are no other suitable criteria. Such a token that can terminate an expression is, for instance, not a binary operator or a message sending operator, which both require further tokens to create an expression. Keywords that expect any following tokens also can not terminate expressions. Gear interpreters and compilers do not look-ahead beyond newlines.
