...
\paragraph{Sources}
A common name for this directory is \code{src}, or \code{source}. Inside of this directory, all Gear sources may be located, or alternatively, it can be forked into more directories, each for a specific language. The directory may then look like:
\begin{lstlisting}
/src
    /c
    /cpp
...
Gear source directory (\code{/src} or \code{/src/gear}) might also contain a build file (or multiple build files, and even along with the language-specific directories in \code{/src} directly), which, for Gear, is named \code{build.gear}, and can make use of other files as needed. 
...
Gear compiled files (bytecode) have the suffix \code{.gearb} for bytecode and \code{.gearpsi} for extracted Program Structure Information. Readme files might be copied into it, if needed. 
