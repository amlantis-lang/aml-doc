...
If the keyword \code{optional} is given before the parameter type, then the parameter type $T$ is wrapped in type ~\lstinline!Option[$T$]!, and instead of \code{undefined}, a missing argument is given as \code{None}, and a present argument $x$ is given wrapped in ~\lstinline!Some($x$)!. 
