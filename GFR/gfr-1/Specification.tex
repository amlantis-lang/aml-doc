...
Like Java VM, a Gear VM should shield users of its programming languages from specifics of each platform as much as possible. E.g. numbers -- each platform has specific representations of numbers, some of which may fit into the languages, some may need to be represented in a more virtual form, like the \code{Decimal} in Gear, which represents virtually any non-complex number digit-by-digit. 
...
A Gear VM should natively support only a single bytecode format -- the Gear VM bytecode. Other bytecode formats may be supported by means of Gear VM extensions, working as a compiler from a custom bytecode into a Gear VM bytecode, primarily to support sort of a transpilation, e.g. from Java's \code{.class} format directly into the Gear VM bytecode format, if such extension is ever built. 
...
This chapter describes the Gear VM Bytecode file format, \code{.gearb}. Each \code{.gearb} file contains definition of a main instruction sequence graph along with additional graphs, which is unlike Java's \code{.class} file format, which only contains definition of a single class. 

A \code{.gearb} file consists of a stream of 8-bit bytes. All 16-bit, 32-bit, 64-bit and 128-bit quantities are constructed by reading in two, four, eight, and sixteen consecutive 8-bit bytes, respectively. Multibyte data items are always stored in big-endian order, where the high bytes come first. 
...
A \code{.gearb} file consists of a single \code{File} structure: 
...
\begin{description}[font=\ttfamily\mdseries,style=nextline]
  \item[magic]
  The \code{magic} item supplies the magic number identifying the \code{.gearb} file format; for the Gear VM bytecode, it has the value of \code{0xC0124189}. 
  \item[major\_version, minor\_version]
  The \code{major_version} along with the following \code{minor_version} items determine the version of the bytecode file format. A particular Gear VM may only support file formats in a certain range. 
  \item[dialect]
...
\begin{description}[font=\ttfamily\mdseries,style=nextline]
  ...
  \item[element\_types]
  The \code{element_types} array contains bytes that mark the type of each element in the \code{elements} array. The valid values of these bytes are listed in \autoref{table:cp-type-path-element-types}. This array is necessary, because there is no possible sub-language that would express a type path with a special syntax within a single byte array, because a Gear VM supports identifiers made by any \mbox{UTF-8} strings. 
  \item[elements]
...
\begin{table}[!h]
  ...
  \begin{tabular}{ l r p{6.5cm} }
    ...
    \code{Type_Path_Projection}       & 9   & not present in \code{elements} \\ \hline % this converts the previous elements into something like Gear's (e).type, and the following t is then (e).type#t
...
A type path can also refer to a type that is made available with a type scope, which includes mechanism that can mark a scope being a parent scope of another scope (this is important for reuse of type scopes -- and type scopes can possibly be reused across different bytecode files as well). When talking Gear, this can be done by using \code{use} clauses to import names into a particular scope -- such types are then available with type paths that start with a selection symbol. 
...
A language has to define the order of search for a type, since a type scope is not the only place where it may look for. The way it defines that is irrelevant to Gear VM, and it does so by implementing the language bindings (\sref{sec:language-bindings}). 
...
% TBD: as the implementation of a reference Gear VM progresses, fill in details
