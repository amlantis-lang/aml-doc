...
\subsection{The \code{Const_Type_Path_Item} Structure}
...
The \code{Const_Type_Path_Item} structure is:
\begin{lstlisting}
...
The items of the \code{Const_Type_Path_Item} are the following:
\begin{description}[font=\ttfamily\mdseries,style=nextline]
  \item[tag]
  The \code{tag} item of the \code{Const_Type_Path_Item} structure has the value \code{Const_Type_Path} ($2$). 
  \item[length]
...
\subsection{The \code{Const_Type_(U)Int$n$_Item} Structures}

A CVM provides for its bytecode a rather wide range of integral types for constants, including both signed and unsigned versions. Numbers that fall out of the scope of these types or numbers that have to be as precise as possible have the \code{Const_Type_Decimal_Item} structure available (including real numbers).
...
The presence of a sign bit in each structure is determined by its tag: \code{Intn} items are signed, \code{UIntn} items are not signed. Unsigned versions of these structures have the same layout. 





\subsection{The \code{Const_Type_Float$n$_Item} Structure}
...
The \code{Const_Type_Float$n$_Item} structures are:
