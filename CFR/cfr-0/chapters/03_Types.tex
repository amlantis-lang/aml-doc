...
Coral offers two annotations that can constrain a type to either {\em mutability} or {\em immutability}. Implicitly, types are not constrained in such a way, therefore a type could represent both mutable and immutable values at the same time. 
...
A {\em foreign type} is a tool that helps Coral programs integrate with other languages written for the same VM, the CVM. It is viewed as extending the type \code{Any}, can be given an alias name and be a part of a compound type with another foreign types or Coral types. 

\paragraph{Syntax note}
If an opening delimiter ``\lstinline!{!'' appears within the foreign language type name, then it is properly paired with the next ``\lstinline!}!'' by Coral and not treated as closing delimiter, but this is done without any knowledge of the foreign language, only presuming that these delimiters are paired. 

\paragraph{Parameterized foreign types}
If the foreign type is parameterized, it can be a part of a type application as well, but its type parameters are not declared neither defined on the Coral side. Also, the foreign language defines how the parameterization works -- e.g., Java does type erasure with its type parameters.
...
;; also:
Simple_Type ::= 'Lang~[coral].Predef.Variadic_Arguments'
\end{lstlisting}
...
if one has $T \conforms T'$, and $L_i \conforms L'_i$ and $U_i \conforms U'_i$ for $i = 1 \commadots n$. 
  ...
  \item A union type (\sref{sec:unions}) $U_1$ conforms to $U_2$, if every member type that is present in $U_2$ has also an equivalent member type in $U_1$. $U_1$ may thus contain more member types than $U_2$, but must contain all of member types in $U_2$. 
...
\begin{enumerate}
  ...
  The type has just one inhabitant, and that is the frozen \code{nil} value. In a Coral VM, this type is usually an immediate value. 
  ...
  The type has just one inhabitant, and that is the frozen \code{undefined} value, which is equal to, but not identical to \code{nil}. In a Coral VM, this type is usually an immediate value.
  ...
  The type has just one inhabitant, the ``\code{()}'' value, also known as an empty tuple. In a Coral VM, this type is usually an immediate value.
