...
A clone constructor is implicitly added to every case class, unless the case class already has a matching one, or the class has a repeated parameter. The clone constructor is defined as follows:
\begin{lstlisting}
clone :[$\tps$][<$\uomps$>]($\ps'_1$)$\ldots$($\ps'_n$): $c$:[$\tps$][<$\uomps$>]
  cloned.$xs_{11}$ := self.$xs_{11}$
  $\ldots$
  cloned.$xs_{nk}$ := self.$xs_{nk}$
end
\end{lstlisting}
...
Trait_Def      ::= constant_id [Type_Param_Clause] Trait_Tmpl_Env
