...
\begin{enumerate}
  \item A dispatch function is defined with ~\lstinline!def dispatch $x\,\ldots$! (or any alternative syntax using the \code{dispatch} keyword). This function returns a value or a tuple of values, which determine the multi-method to use, but its result type is independent of this value. 
