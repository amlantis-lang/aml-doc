...
A reference to any other member of the \code{nil} object causes \code{Member_Not_Found} to be raised, unless the member in fact exists. The \code{nil} object is not frozen by default, but the CSL guarantees to never edit it. 
...
The \code{undefined} object is frozen by default; it is not equal to \code{nil}, but \code{Nothing === undefined} results in \code{yes} and \code{Undefined === nil} results in \code{no}. The recommended approach for testing whether an expression results in \code{undefined} is using typed expression: \code{maybe_value is not Undefined} or \code{maybe_value is Undefined}. Explicit literal use of the \code{undefined} object should be avoided when possible, and definitely not used in place where \code{nil} is sufficient. 

Implicit conversions to the \code{Undefined} type are forbidden. The \code{Undefined} type is also forbidden to appear in type bindings (except for typed expressions, obviously). 
...
\rweight(t) &= \left\{ 
  \begin{array}{l l}
    1 & \textrm{unless $t$ is a variadic or a capturing namd parameter}\\
    0 & \textrm{otherwise}
  \end{array} \right.
\end{array}\]
\end{definition}
