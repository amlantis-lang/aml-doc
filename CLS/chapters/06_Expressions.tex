...
A parameterless method $m$ of type ~\lstinline!() -> $T$!~ is always converted to type $T$ by evaluating the expression to which $m$ is bound, unless method value (\sref{sec:method-values}) is used on $m$. 
...
Assume that $e$ appears as a function in an application, as in ~\lstinline!$e$($e_1 \commadots e_m$)!.
