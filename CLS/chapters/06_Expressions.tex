...
The {\em associativity} of an operator is determined by the operator's last character. Operators ending in a colon ``\lstinline!:!'' are right-associative. All other operators are left-associative. 
...
\item If there are consecutive infix operartions of the same precedence, then all those operators must have the same associativity (i.e. it is an error if they don't). % TBD: finish this item
...
Broken_Expr    ::= 'broken' Broken_Expr
\end{lstlisting}
