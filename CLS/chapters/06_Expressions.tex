...
Simple_Expr1 ::= [Container_Path '.'] 'self'
               ...
               | [Container_Path '.'] 'self' '[' 'cloned' ']'
                 '.' (ivar_id | Selection)
               | [Container_Path '.'] (ivar_id | cvar_id)
\end{lstlisting}
...
\item A left-associative binary operation $e_1\ \op\ e_2$ is interpreted as ~\lstinline!$e_1$.`$\op$`($e_2$)!. If $\op$ is right-associative, the same operation is interpreted as ~\lstinline!{ val x := $e_1$; $e_2$.`$\op$`($x$) }!, where $x$ is a fresh name. If $\op$ is ``\lstinline!=>!'' (right-associative), the operation is interpreted as ~\lstinline!Pair($e_1$, $e_2$)!, where \code{Pair} is a simple relative name (overrideable in the scope).
