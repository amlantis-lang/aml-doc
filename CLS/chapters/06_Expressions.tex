...
We say that a type $T$ is {\em compatible} to a type $U$ if $T$ weakly conforms to $T$ after applying eta-expansion (\sref{sec:eta-expansion}) and view applications (\sref{sec:views}).
...
A parameterless method $m$ of type ~\lstinline!() -> $T$!~ is always converted to type $T$ by evaluating the expression to which $m$ is bound, unless method value (\sref{sec:method-values}) is used on $m$. This evaluation (as well as other method conversions) happens when the expression is to be evaluated (see argument evaluation strategies), so the expected type of a parameterless method can be converted to be an expected type $T$. 
...
It is an error if there is no alternative in $\mathcal{C}$ which is more specific than all other alternatives in $\mathcal{C}$ -- the method is unapplied. 
...
Assume finally that $e$ does not appear as a function in either an application or a type application. If an expected type is given, let $\mathcal{B}$ be the set of those alternatives in $\mathcal{A}$ which are compatible (\sref{sec:implicit-conversions}) to it. Otherwise, let $\mathcal{B}$ be the same as $\mathcal{A}$. It is an error if there is no such alternative. If there is one such alternative, that one is chosen. 
...
It is an error if there is no alternative in $\mathcal{B}$ which is more specific than all other alternatives in $\mathcal{B}$ -- the method is unapplied. 

% TBD: maybe add more examples? including named arguments, optional arguments...

% TBD: decide whether overloading is needed in other cases than function application - guess is that not, since Coral should not allow such bad things such as a function value and a function that is override-equivalent to that value function in the same scope, so the same goes with simple parameterized types and in no way with just expected type (like what, decide on a member based on what the caller is expecting? no.) // check http://stackoverflow.com/questions/7768669/overloading-a-method-based-on-a-type-parameter
