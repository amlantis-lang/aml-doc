...
A singleton type is of the form \lstinline@$p$.type@ and a special type that denotes the set of values consisting of \code{nil} and the value denoted by $p$, in spite of nullability.
...
String@[42]
\end{lstlisting}
Note that ~\lstinline!Integer@[42]!~ is basically equivalent to ~\lstinline!42.type!. 
...
\item
Two method types (\sref{sec:method-types}) are equivalent, if they have equivalent return types, both have the same number of parameters and corresponding parameters have equivalent types and extra properties. Names of the parameters matter for method type equivalence only with named parameters. 
...
\end{itemize}
% TBD: conformance of dependent types, important! 
