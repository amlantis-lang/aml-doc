...
\end{lstlisting}
consists of a selector expression $e$, a number $n > 0$ of cases and an optional ``default'' case, denoted with \code{else} keyword. Each case consists of a (possibly guarded) pattern $p_i$ and a block $b_i$, or just the block $b_{n+1}$. Each $p_i$ might be complemented by a guard \code{if e} or \code{unless e}, where $e$ is a the guarding expression that is evaluated as boolean. The scope of the pattern variables in $p_i$ comprises the pattern's guard and the corresponding block $b_i$. 
