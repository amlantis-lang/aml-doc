...
First, {\em variable identifiers}, which suggest that the identifier represents a value or a variable. 

Second, {\em constant identifiers}, which suggest that the identifier represents a constant value, or a type.

Third, {\em instance variable identifiers}, which are used to distinguish between local values, variables and instance variables. Those start with ~\lstinline!@!~ for instance variables and  ~\lstinline!@@!~ for class object variables. 
...
loop          match         memoize       message       method
module        native        next          nil           no
object        of            opaque        operator      out
override      pointcut      pragma        prepend       private
property      protected     protocol      public        raise
raising       range         record        redo          refine
refinement    rescue        retry         return        returning
requires      reverse       sealed        self          set
skip          step          super         target        then
this          throw         throwing      trait         transparent
type          unless        until         union         unit-of-measure
