...
% invent syntax for something that retrieves a type out of a value, so that the type will represent either nothing or the single value (singleton type), and possibly invent different syntax that will retrieve a type for all values interchangeable with that one value or nothing. #type #singleton_type or #singleton-type
...
A type projection \lstinline@$T$#$x$@ references type member named $x$ of type $T$. 

Type projection operator \lstinline@#@ is a language construct and can't be overridden by user programs. There is a similarity between this construct and the \lstinline@::@ scope operator. The difference is, type projection operator is expected to be rarely needed, but it does provide a type projection and can refer in a stable way to a type of anything. Scope operator, on the other hand, does not care about types, it merely (dynamically) resolves a member of a particular expression. 

\example Difference between scope and type projection operators. 
...
In the above code, the variable \code{a} is a reference to \lstinline@::Boolean::class@ method, the member method \lstinline@.class@ of every \code{Boolean} instance. On the other hand, the variable \code{b} is \lstinline@::Boolean@ class itself. Finally, the variable \code{c} is the class of that instance method. 
