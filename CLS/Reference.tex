...
To retrieve the actual type of a value, the method \lstinline@.class@ can be used. This method is defined on every object and its return type is covariant -- it is always a \lstinline@Class@ (or a subtype, if any exists). Type projection to ``\lstinline@class@'' can be used as a stable way to reference the real class of any expression that has a value, referred to as ``\lstinline@Value_Expr@'' in the following syntax. This type then applies only to the actual value of the expression, which is only known at runtime, and as such may change with each evaluation. 

\syntax\begin{lstlisting}

Simple_Type ::= Path '.' 'class'
Simple_Type ::= Simple_Type '##' 'class'
Simple_Type ::= Value_Expr '##' 'class'

\end{lstlisting}

\example Sample value type references. On the second line, the referenced type is a union type of both \lstinline@a@'s and \lstinline@c@'s types. 
