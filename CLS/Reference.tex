...
operator     out         prepend      private      property
protected    protocol    public       raise        range
record       redo        refine       rescue       retry
return       requires    self         set          singleton-type
...
Clone constructors are pretty much like constructor, except for they are not invoked indirectly by \code{allocate} on \code{Class}, but by \code{clone} on the cloned instance. 

Clone constructor implicitly returns the new cloned object, unless returning explicitly a different object. The original object is available with the \code{self} and \code{this} keywords, the new cloned object is available as the \code{clone} keyword. 
...
class $C$
  clone
    // super is invoked implicitly here
    clone.resource := self.resource.clone
  end constructor
end class
\end{lstlisting}
...
\syntax\begin{lstlisting}
Modifier ::= 'public'
           | 'protected' [Access_Qualifier]
           | 'private' [Access_Qualifier]
           | 'implicit'
           | 'lazy'
Access_Qualifier ::= ':[' (constant_id | 'self') ']'
\end{lstlisting}

Modifiers may appear in two forms:
...
By default\footnote{That is, without any explicit modifier being applied.}, the \code{public} modifier affects every member of the class type, except for instance variables and class instance variables, which are object-private (\lstinline!private:[self]!). 
