...
When a mixin $P$ is prepended to a class, a new include class $Ip_i$ is injected between the target class and its last prepended mixin, if any. Prepended mixins are stored in a secondary inheritance chain just for prepended mixins, forming an inheritance tree. Every class that has $P$ prepended is available via the \code{prepended_in} method of $P$. 
...
The order and rules of function resolution are further explained in \sref{sec:function-applications}. 
