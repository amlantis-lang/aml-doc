...
clone       cloned      constant    constructor declare
def         destructor  do          else        elsif
end         ensure      enum        for         for-some
function    get         goto        if          implements
implicit    in          include     interface   is
lazy        let         loop        match       memoize
message     method      mixin       module      native
next        nil         no          object      of
opaque      operator    out         prepend     private
property    protected   protocol    public      raise
range       record      redo        refine      rescue
retry       return      requires    self        set
singleton-type          skip        step        struct
super       template    test        then        this
throw       transparent type        undef       unless
