...
Quantification_Dcl     ::= Type_Definition (* containing Type_Abstract *)
                         | Val_Dcl
Compound_Type          ::= Annot_Type [Nullable_Mod] {['prepend'] 'with' Annot_Type} 
                           ['with' ['refinement'] Refine_Stats]
                         | ['refinement'] Refine_Stats
Postfix_Type           ::= Compound_Type 
                           ['with' 'constraint' Constraint_Block]
Infix_Type             ::= Postfix_Type 
                           {[nl] (Infix_Op - 'also', 'else') [nl] Postfix_Type}
...
And now, move on to Aml. Aml uses hybrid typing, an approach sometimes called gradual typing. In its core, it uses static typing. But, it allows to opt-in for dynamic typing using a special type \code{Dynamic}. It even allows to combine the dynamic type with static constraints. Unlike in Ruby, you can overload methods (not just override!). You can constrain variables, constants, properties, arguments and result types to particular types. But you don't have to. 
...
When a type is expected somewhere, but it is not given, be it a type constraint or a type argument, Aml makes attempts to infer the type in a best way it can (\sref{sec:automatic-inference}). 
...
Types in Aml are represented by objects that are members of the \code{Type} type. Instances of value types are represented by objects that are members of the \code{Class} type. Aml can also refer to types coming from other languages, but then these are treated much like the \code{Dynamic} type, unless there is special tooling available for the particular foreign language. 
...
Class_Qualifier 
    ::= '[' (id | Simple_Type) ']'
Module_Path 
    ::= [Root_Module] Module_Selector {'.' id}
Module_Selector 
    ::= [Vendor] id
Root_Module
    ::= '@Root' '.'
...
Type_Arg            ::= Type_Arg_Value
                      | Measure_Arg_Value
                      | Static_Arg_Value
Type_Arg_Value      ::= ['~' id ':'] Type
                      | '_'
Measure_Arg_Value   ::= ['~' id ':'] '<' measure_literal '>'
Static_Arg_Value    ::= Constant
                      | 'constant' '(' Expr ')'
\end{lstlisting}
...
\grammar\begin{lstlisting}
Function_Type 
    ::= [Implicit_Types] Function_Args {'->' Function_Args}
        ['->' Signalling_Types] '->' Type
Function_Args 
    ::= Infix_Type
      | '(' [Function_Arg {',' Function_Arg}] ')'
...
Given_Dcl ::= ...
            ...
            | Type_Var '<:' Type
            | Type_Var ':' 'nil'
            | Type_Vars ':' '(' Member_Signature ')'
            | Type_Var ':' '(' Constructor_Signature ')'
Type_Vars ::= Type_Var
            | '(' Type_Var {'or' Type_Var} ')'
Member_Signature
          ::= ['class'] 'member' fun_id ':' Type
            | ['class'] 'operator' op_id ':' Type
            | ['class'] 'property' id ':' Type ['with' Property_Signature_Extra]
Constructor_Signature
          ::= ('new' | 'constructor') ':' Type  (* just type of parameters, no result type *)
...
\grammar\begin{lstlisting}[escapechar=@]
Type
    ::= ...
      | Infix_Type Quantification_Clauses
Quantification_Clauses 
    ::= {'for-some' '{' Quantification_Dcl
        {semi Quantification_Dcl} '}'}
Quantification_Dcl
    ::= Type_Definition (* containing Type_Abstract *)
      | Val_Dcl
...
If an implicit conversion from \code{Nothing} to $T$ is available, then nullability is not a problem. Also, if $T$ implements the \code{Default[$T$]} trait (\sref{sec:types-preventing-nil}), nullability is also not a problem, as there will always be a value that replaces \code{nil} or even \code{undefined}. 
...
A strictly not-nullable type has the form ~\lstinline@$T$!@, where ``\code{!}'' denotes a not-nullable type. Aml types are implicitly not-nullable, but strictly not-nullable types have the extra property that \code{nil} and \code{undefined} do not cause a warning when assigned to a variable or value that is of a strictly not-nullable type, but raise a type error. In case the referenced value changes in runtime by other means than assignment (native access or maybe due to hot code load), the same error has to be raised. 

A compiler may decide to issue extra warnings when assignment of a \code{nil} or \code{undefined} can be detected. 

A strictly not-nullable may be seen as some implicitly wrapped-on-assignment / unwrapped-on-read box that checks for \code{value = nil} and raises an error when such condition is true. But Aml does not have any special methods for object dereference.\footnote{Yet, and probably never will.} 
...
In Aml, two traits exist that make every type that implements them conform to a compatible strictly not-nullable type: \code{Default[$T$]} and \code{Never_Nil}. 

\code{Default[$T$]} does that by implementing a default value to runtime-replace \code{nil} or \code{undefined}. \code{Never_Nil} does that by making the implementing type behave as if all of its occurrences were in fact \code{$T$!} (strictly not-nullable). 

An implementation of \code{Default[$T$]} must provide the following method, where \code{self} is the implementing class or type instance: 
\begin{lstlisting}
message self.default (): $T$! end
\end{lstlisting}

Then, the value returned by it replaces occurrences of \code{nil} or \code{undefined} in runtime. Also, an attribute is provided for the implementing type, in form of \code{$T$.'default}. 
...
Foreign_Type ::= 'foreign' lang_name string_literal
lang_name    ::= id
\end{lstlisting}
...
\begin{enumerate}
  ...
  The type has just one inhabitant, and that is the frozen \code{nil} value. In a Aml VM, this type is usually an immediate value. 
  ...
  The type has just one inhabitant, and that is the frozen \code{undefined} value, which is equal to, but not identical to \code{nil}. In a Aml VM, this type is usually an immediate value.
  ...
  The \code{None} case object is primarily designed to be used with pattern matching, and also plays an important internal part in Aml's implementation of pattern matching. 
