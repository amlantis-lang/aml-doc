...
A type \lstinline!'$x$! stands for a type variable named $x$. The associated type is \code{Aml/Language.Auto}, for use with automatic inference (\sref{sec:automatic-inference}).
...
Simple_Type         ::= Simple_Type [Type_Args]
                      | Postfix_Simple_Type
Postfix_Simple_Type ::= Type Stable_Id
Type_Args           ::= '[' [Type_Arg {',' Type_Arg} [',' '...' id]] ']'
                      | '[' '...' id ']'
...
\subsection{Type Dependencies \& Equations}

\grammar\begin{lstlisting}
Given_Dcl ::= Type_Var Type_Equation
            | Type_Var '>:' Type ['<:' Type]
            | Type_Var '<:' Type
\end{lstlisting}
...
\grammar\begin{lstlisting}[morekeywords={Auto},deletekeywords={also}]
Simple_Type ::= 'Auto'
(* also: *)
