...
An enum constant can be enhanced with custom parameters, which are then passed to the appropriate custom constructor. Enum definitions should only appear in four forms -- the first three of them are called {\em constant enum constructors}, the last is called {\em non-constant enum constructor}. 
\begin{itemize}
  ...
  \item Initialized in-place with custom expression, according to the specified types, whose members may be extracted with pattern matching (\sref{sec:named-patterns}). Those enums are similar to variant types (\sref{sec:variant-types}), but the enumerated members are nested in the enum class and thus have to be prefixed, or imported explicitly into scope. 
\end{itemize}
...
If an enum type is specified with the \code{bitfield} modifier, then the contained values can be only positive integral numbers, and those fields that are not given an explicit value are given a value that is the immediately following power of two, or \code{1} for the first field. 
