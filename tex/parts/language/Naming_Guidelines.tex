...
Macros should be placed in a \code{/macros} directory inside of a directory with Aml sources, so e.g. \code{/src/macros} or \code{/src/aml/macros}. Those files may also use other files as needed. A file that contains solely Aml macro definitions may be optionally named with suffix \code{.amlm}, but that is not a requirement. 
...
Aml protocol files have the suffix of \code{.amlp}, but their structure is identical to any other Aml source file. 
