...
A variable pattern is implicitly prefixed with \code{val}, and thus the bound variable is immutable, and prefixing it explicitly is redundant. Use the \code{val} prefix in cases where the bound name would shadow another name, making the pattern ambiguous, which would be an error. 
