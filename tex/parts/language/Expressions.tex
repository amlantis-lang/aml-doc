...
Tuple_Exprs 
    ::= Expr {',' Expr}+
      | Tuple_Head_Expr '(' [nl] Expr {',' Expr}+ [nl] ')'
Tuple_Head_Expr
    ::= [Mutability_Modifier] 'tuple' [nl]
Constant_Expr 
    ::= ...
      | '()'
\end{lstlisting}

A tuple expression ~\lstinline!$e_1 \commadots e_n$!~ is an alias for the class instance creation ~\lstinline!Tuple($e_1 \commadots e_n$)!, where $n \geq 2$. The empty tuple ~\lstinline!()!~ is the unique value of type \code{Unit}. A tuple with only one value is usually only the value itself, without being wrapped in a tuple.  
...
\grammar\begin{lstlisting}
Function_Expr
    ::= 'fun' {Anonymous_Params_Clause}+ [Guard] [':' Type] 
...
Use_Expr 
    ::= Use_Expr_As 
      ...
      | Use_Refinement
      | Use_Droppable
      | Import_Clause [Do_Block | In_Sep Expr ['end']]
      | 'let' Open_Clause In_Sep Expr ['end']
      | Local_Open_Expr
Use_Expr_As 
    ::= 'use' Expr ('as' | 'as!' | 'as?')
        [id ':'] Type [Do_Block | In_Sep Expr ['end']]
Use_Aspect
...
Binding_Expr 
    ::= ['implicit'] Val_Def In_Sep Expr ['end']
      ...
      | 'let' Struct_Defs In_Sep Expr ['end']
\end{lstlisting}
...
When_Clause 
    ::= 'when' Pattern [Guard] ('then' | semi) When_Expr
When_Expr
...
Rescue_Clauses 
    ::= Rescue_Clause {semi Rescue_Clause}
Rescue_Clause 
    ::= 'when' Pattern [Guard] ('then' | semi) Rescue_Block
Rescue_Block
