...
Tuple_Field_Init  ::= (id | decimal_numeral) '=>' Expr
\end{lstlisting}
...
\grammar\begin{lstlisting}
Tuple_Exprs ::= ['mutable' | 'immutable'] 'tuple' [nl] 
                '{' [nl] Expr 'with' Tuple_Fields_Clone [nl] '}'
Tuple_Fields_Clone 
...
\grammar\begin{lstlisting}
Simple_Expr         ::= Record_Clone_Expr
Record_Clone_Expr   ::= 'record' [nl] 
                        '{' [nl] Expr 'with' Record_Fields_Clone [nl] '}'
Record_Fields_Clone ::= Record_Field_Clone {semi Record_Field_Clone}
Record_Field_Clone  ::= Stable_Id ['=>' Expr]
\end{lstlisting}
...
Arguments expressions of the form ~\lstinline!arguments $a_1 \ldots a_n$!~ generate a special value, which is equivalent to what is passed as arguments in function applications. % TBD: define what the type is; also define the operations on it (joining arguments lists etc.)
...
Workflow ::= 'let!' Let_Binding semi Workflow
           ...
           | ['implicit'] Fun_Def In_Sep Workflow ['end']
           | ['implicit'] Op_Def In_Sep Workflow ['end']
           | ['implicit'] Fun_Dcl In_Sep Workflow ['end']
           | ['implicit'] Op_Dcl In_Sep Workflow ['end']
           | ['implicit'] Multi_Fun_Def In_Sep Workflow ['end']
           | ['implicit'] Multi_Op_Def In_Sep Workflow ['end']
           | ['implicit'] Multi_Fun_Dcl In_Sep Workflow ['end']
           ...
           | 'use!' [S_Modifier] Let_Binding In_Sep Workflow ['end']
           | 'use' [S_Modifier] Let_Binding In_Sep Workflow ['end']
           | 'yield!' Expr
           | Yield_Expr
           | 'return!' Expr
           ...
           | 'if' Wf_Condition ('then' | semi) Wf_Cond_Block
             {[semi] 'elsif' Wf_Condition ('then' | semi) Wf_Cond_Block} 
             [[semi] Else Wf_Cond_Block] 'end' ['if']
           | 'unless' Wf_Condition ('then' | semi) Wf_Cond_Block
             {[semi] 'elsif' Wf_Condition ('then' | semi) Wf_Cond_Block} 
             [[semi] Else Wf_Cond_Block] 'end' ['unless']
             
           | ? Match_Expr where Cond_Block is extended with Wf_Cond_Block ?
           | ? Catch_Expr or Rescue_Expr, where initial Block is extended with Workflow_Block
               and Handle_Block, Catch_Block and Rescue_Block with Workflow ?
...
Wf_For_Loop    ::= 'loop' Wf_Loop_Block 'end' ['loop']
                 ...
                 | '{' Wf_Loop_Block '}'
Wf_Loop_Block  ::= {Workflow_Block | Loop_Ctrl_Expr}
Workflow_Block ::= ? Block where Block_Stat and Expr are extended with Workflow ?
Wf_Cond_Block  ::= Expr 
                 | Workflow_Block
Wf_Condition   ::= (Workflow - Cond_Expr)
\end{lstlisting}
...
Any type may implement the protocol \code{Aml/Language.List_Literal_Convertible}, and then list literals may be converted to the type. 
...
Selection          ::= Simple_Selection
                     ...
                     | Optional_Selection
Simple_Selection   ::= id
Tuple_Selection    ::= decimal_numeral
...
\grammar\begin{lstlisting}
Simple_Expr ::= [id '.'] 'self' ['[' ('cloned' | 'origin') ']']
                ['.' Selection]
...
Use_Expr       ::= Use_Expr_As 
                 ...
                 | Use_Refinement
                 | Use_Droppable
Use_Expr_As    ::= 'use' Expr ('as' | 'as!' | 'as?')
                   [['var' | 'val'] id ':'] Type [Do_Block | In_Sep Expr ['end']]
Use_Aspect     ::= 'use' 'aspect' Stable_Id [Do_Block | In_Sep Expr ['end']]
Use_Refinement ::= 'use' 'refinement' Stable_Id [Do_Block | In_Sep Expr ['end']]
Use_Droppable  ::= 'use' [S_Modifier] Let_Binding In_Sep Expr ['end']
\end{lstlisting}
...
Positional_Argument 
            ::= ['?' | '*' | '**'] Expr
              ...
              | Target_Type_Expr (* only when not following an Expr directly *)
Labelled_Argument
            ::= ['~' | '~?'] id ':' Expr
              | ['~']        id ':' Target_Type_Expr
              | ['~']        id ':' Slice_Expr
...
A function application ~\lstinline!$r$($e_1 \ldots e_m$)!, where at least one of $e_i$ is literally ``\,\lstinline[mathescape=false]!$!\,'' and \code{r} is a variable reference, is a syntax sugar for function application, where each ~\lstinline[mathescape=false]!$!~ is replaced with ~\lstinline[mathescape=false]!r.`$`()!. 
...
\paragraph{Note}
The receiver universal shortcut is defined for Aml's collection types as an alias to \code{size} -- to be used in slicing, primarily. 
...
\grammar\begin{lstlisting}
Attribute_Expr ::= Expr '.'' Simple_Selection
\end{lstlisting}
...
\grammar\begin{lstlisting}
Type_Application_Expr ::= Expr Type_Args
\end{lstlisting}
...
\grammar\begin{lstlisting}
Block_Expr      ::= Do_Block | Block_Expr1 | Block_Expr2
...
Do_Block        ::= '{' Block '}' 
                  | 'do' Block Done
Do_Binding      ::= 'do' Expr [In_Sep Expr] [Done]
Done            ::= 'end' | 'done'
\end{lstlisting}
...
Infix_Op     ::= op_id 
               ...
               | 'xor'
               | id
Prefix_Expr  ::= [op_id | 'not' | Try_Op] Expr
Range_Expr   ::= Expr ('..' | '...' | '..<') Expr
                 [FP_Delta [FP_Digits]]
Slice_Expr   ::= (* slice of 1 element *)
                 Expr 
                 (* slice defined by receiver universal shortcut *)
               | '$'
                 (* slice from index (or $) to last *)
               | (Expr | '$') '..'
                 (* slice from first to index (or $) *)
               | ('..' | '...' | '..<') (Expr | '$')
                 (* slice from index (or $) to index (or $) *)
               | (Expr | '$') ('..' | '...' | '..<') (Expr | '$')
...
\grammar\begin{lstlisting}
Assign_Expr ::= [Expr '.'] id ':=' Rvalue_Expr
              | Expr '.' Selection ::= Rvalue_Expr
Update_Expr ::= Expr '(' [Arguments] ')' ':=' Rvalue_Expr
Rvalue_Expr ::= Expr | Target_Type_Expr
\end{lstlisting}
...
Analogously, an assignment ~\lstinline!$f$.$x$ := $e$!~ is interpreted as the invocation ~\lstinline!$f$.set_$x$($e$)!. If $f$ is evaluated to \code{nil}, then the invocation is forwarded to \code{nil}.\footnote{This likely results in a runtime error being raised, unless \code{nil} would actually implement method \code{set_$x$}.} 

An assignment ~\lstinline!$f$.?$x$ := $e$!~ is interpreted as the invocation ~\lstinline!$f$.?set_$x$($e$)!. If $f$ is evaluated to \code{nil}, then the invocation is evaluated to \code{nil}. See (\sref{sec:designators}) for more on behavior of the ``\lstinline!.?!'' navigation. 
...
Def_Expr ::= ['implicit'] Val_Def In_Sep Expr ['end']
           ...
           | ['implicit'] Macro_Dcl In_Sep Expr ['end']
           | ['implicit'] Macro_Def In_Sep Expr ['end']
...
\grammar\begin{lstlisting}
Cast_Expr  ::= Expr ('as' | 'as!' | 'as?') Type
Infix_Expr ::= Expr ('is' ['not'] | 'is!' | 'is' 'not!') Type
\end{lstlisting}
...
Condition        ::= Condition1
                   | Condition2
Condition1       ::= (Expr - Cond_Expr) 
Condition2       ::= Val_Def | Var_Def
\end{lstlisting}
...
The conditional variable definition ~\lstinline!if $p$ := $e_1$ then $e_2$ else $e_3$!, where ~\lstinline!$p$ := $e_1$!~ is a variable binding (see syntactic categories \code{Var_Def} \& \code{Val_Def}), works the same way as other conditional expressions, with the following difference: if $e_1$ evaluates to \code{nil}\footnote{Or \code{undefined}, which is an equal case.} or even \code{()} (\code{Unit} value), then the condition is regarded as \code{no}. Beware that if $e_1$ evaluates to \code{no} (literally), the condition is still met (because \code{no} is not \code{nil} neither \code{()}) and $e_2$ is evaluated. Note that if type required by $p$ does not allow \code{nil} or \code{()} value, then no error happens and $e_3$ is evaluated instead. The condition in this case says ``if the variable definition ~\lstinline!$p$ := $e_1$!~ can be realized, then proceed with $e_2$, otherwise proceed with $e_3$''. In case the \code{unless} keyword is used instead of \code{if}, then the variable binding is effective in $e_3$ (and possible other \code{elsif} branches), but not in $e_2$.
...
\grammar\begin{lstlisting}
Loop_Expr ::= 'for' id ':=' Expr ('to' | 'downto') Expr For_Loop
\end{lstlisting}
...
\grammar\begin{lstlisting}
Catch_Expr     ::= Catch_Expr1 | Catch_Expr2 | Handle_Expr1 | Handle_Expr2
...
Handle_Clauses ::= Handle_Clause {semi Handle_Clause}
                   [semi Else Handle_Block]
Handle_Clause  ::= 'when' Pattern [Guard] ('then' | semi) Handle_Block
Handle_Block   ::= Expr | {Handle_Stat semi} [Result_Expr]
Handle_Stat    ::= Block_Stat | Resignal_Expr
Resignal_Expr  ::= 'resignal' [Cond_Modifier1]
\end{lstlisting}
...
Template_Stat  ::= ? Expr, which includes the following extra elements for Expr:  ?
                 ...
                 | {Modifier} (Def_Expr - ('implicit', Local_Modifier))
                 | {Modifier} Val_Def
                 | {Modifier} Var_Def
                 | {Modifier} Fun_Def
                 | {Modifier} Op_Def
                 ...
                 | ['inherit' 'and'] 'prepend' Long_Id [Cond_Modifier]
                 | 'implements' Long_Id [Cond_Modifier]
                 | Expr
                 | Alias_Expr
                 | Struct_Expr
                 | Struct_Spec
...
Capture_Items  ::= (id {',' id} | Let_Binding) 'as' R_Modifier
...
Aml defines a marker trait \code{Dynamic_Member_Selecting} that enables dynamic invocations rewriting without resorting to use error handling mechanisms to implement dynamic dispatch. 
