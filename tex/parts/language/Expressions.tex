...
Tuple_Exprs  ::= Named_Expr {',' Named_Expr}
Named_Expr   ::= [['~'] id ':'] Expr
Constant     ::= '()'
\end{lstlisting}
...
\begin{itemize}
  ...
  \item Expression of the form ~\lstinline!%w[word word2 wordx]!
    has type of \code{Linked_List[String]}, and respectively for the other flags, where $T$ is \code{String} (\code{Gear/Language.String}). 
\end{itemize}
...
\begin{itemize}
  ...
  \item Expression of the form ~\lstinline!%m[|$e_1$; $\ldots$; $e_n$|]!
    has type of \code{Mutable_Array[$T$]}.
  \item Expression of the form ~\lstinline!%w[|word word2 wordx|]!
    has type of \code{Mutable_Array[String]}, and respectively for the other flags, where $T$ is \code{String} (\code{Gear/Language.String}). 
\end{itemize}
...
\begin{itemize}
  ...
  \item Dictionary expression that uses the \code{Dict_Mapping1} syntax category to define its elements has $K$ of \code{Symbol} (\code{Gear/Language.Symbol}). 
\end{itemize}
...
\begin{itemize}
  ...
  \item Multimap expression that uses the \code{Dict_Mapping1} syntax category to define its elements has $K$ of \code{Symbol} (\code{Gear/Language.Symbol}). 
\end{itemize}
...
The \code{Gear/Language.Reference_Cell} instance is not automatically unboxed. The contained value can be retrieved from a reference cell using the prefix operator ``\lstinline@!@''\footnote{This operator is not used in meaning of \code{not}, because accidentally overlooking it does not invert the value that it's applied to.}, or by sending the instance a ``\code{value}'' message. The contained value can be overwritten by using the infix operator ``\lstinline!<-!'', or by sending the instance a ``\code{value_=}'' message. 
...
\subsection{Method Values \& Explicit Partial Applications}
...
Since simple identifiers that may make up the \code{Value_Expr} can contain trailing apostrophes, the last apostrophe is considered a part of the attribute selection expression and not the identifier itself. 
...
% TBD: describe last block arg
% TBD: describe arg prefixed with a dot (e.g. .Red)
