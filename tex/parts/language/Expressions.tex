...
High_Precedence_Application_Expr
    ::= Expr [Block_Argument]
      | Expr ? no whitespace ? '(' [Arguments] ')' [Block_Argument]
...
% TBD: fix the following text according to the revised grammar
...
There is one more way to create an instance of a class, and that is to use a syntax construct of the \code{New_Expr} syntax category. This construct has multiple forms for different use cases. 
...
Infix_Op     
    ::= op_id 
      ...
      | 'or' ['else']
      | 'xor'
      | id  (* must exist in scope as an infix operator *)
Prefix_Expr 
...
\begin{itemize}
  ...
  \item A left-associative binary operation $e_1\ \op\ e_2$ is interpreted as ~\lstinline!($\op$) $e_1\ e_2$!. If $\op$ is right-associative, the same operation is interpreted as ~\lstinline!{ val x := $e_1$; ($\op$) $x\ e_2$ }!, where $x$ is a fresh name. 
...
Assign_Expr 
    ::= [Expr '.'] id ':=' Rvalue_Expr
      ...
      | Val_Def  (* only if the pattern binds exactly 1 variable *)
Update_Expr
   ::= Expr '.' '(' [Arguments] ')' ':=' Rvalue_Expr
     | '(' Regular_Precedence_Application_Expr ')' ':=' Rvalue_Expr
Rvalue_Expr 
    ::= Expr 
      | Target_Type_Expr
\end{lstlisting}
...
Type_Ref_Expr 
    ::= '(' 'type' Type ')'
      | Annot_Type - (Type_Var, Postfix_Simple_Type)
\end{lstlisting}
