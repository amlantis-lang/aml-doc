...
$1 + 1                      fun x -> x + 1
$1 * $2                     fun x -> y -> x * y
($1 as Integer) * 2         fun (x: Integer) -> x * 2
if $1 then x else y end     fun (z: Boolean) -> if z then x else y end
$1.map f                    fun x -> x.map f
$1.map ($1 + 1)             fun x -> x.map (fun y -> y + 1)
\end{lstlisting} 
...
Pattern matching is described in (\sref{sec:pattern-matching}). Here, the grammar of expressions that make use of pattern matching is given.
