...
We define a `plain symbol' to be the result of a parse that was not prefixed by ``\lstinline!#~!'', and is not enclosed in matching ``\lstinline!``!''.
...
Alternatively, whenever a pair of two sub-forms is expected, a pair form can be used, see \nameref{subsec:aml-base-lang-reader-lists}.\footnote{This is the basic form of record expressions.}
...
Upon reading a ``\lstinline!'!'', the reader recursively reads next form. If that form is a symbol, a type variable is formed.\footnote{A type variable is a symbol with metadata attached that is yet TBD.} Otherwise, a new list is formed containing the symbol ``\lstinline!#'quote!'' and the following form. 
...
A ``\lstinline!`!'' starts either a quasiquote (see \nameref{subsec:aml-base-lang-reader-quotes}), or a polymorphic tag. If the form immediatelly following it is a plain symbol, a polymorphic tag is read.

\example An example of a polymorphic tag:

\begin{tabular}{ l l }
  \lstinline!`RGBA! & reads equal to \lstinline!(polymorphic-variant-tag #'RGBA)! \\
\end{tabular}
...
A ``\lstinline!~!'' starts a label. The parsing is the same as for a symbol preceded by ``\lstinline!#~!'', except for this initial character, and the part after this initial sequence is never parsed as a number. 

\example An example of parsing a label:

\begin{tabular}{ r l }
  \lstinline!~f! & reads equal to \lstinline!(string->label "f")! \\
\end{tabular}
...
The sequence ``\lstinline!#^!'' starts a metadata. When the reader encounters it, it recursively reads next form, which is expected to be a map without the leading ``\lstinline!#!'', or a plain symbol. In case of a plain symbol $s$, the metadata is constructed as \lstinline!#{(#:metadata . $s$)}!. After the metadata $m$ is read, the reader attaches it to the next recursively read form $f$: \lstinline!(with-metadata $f$ $m$)!.
