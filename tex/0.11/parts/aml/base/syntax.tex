...
The \lstinline@|@ character starts a subsequence of characters to be included verbatim in the delimited sequence. It is never treated as a delimiter. The subsequence is terminated by another \lstinline@|@, and neither the first nor the last \lstinline@|@ is a part of the subsequence. See \nameref{subsec:aml-base-lang-reader-symbols} for details.

% FIXME: let’s not repeat previous Lisps’ mistakes and use @"" instead of || for the verbatim character sequences, in the "aml" flavour of the default readtable. It will allow us the same set of features and then some (think interpolation, but in symbols!), so it would be a superset of R7RS’s capabilities. This would also align more nicely with Aml (high level) and is already used by Zig, so it’s not a completely unknown syntax. 

The \lstinline@\@ outside of a \lstinline@|@ pair causes the following character to be included verbatim in a delimited sequence. The \lstinline@\@ inside of a \lstinline@|@ pair causes the following \lstinline@|@ to be included verbatim in a delimited sequence (instead of matching with the first \lstinline@|@). 
...
\pagebreak[2]
  ...
  ``\lstinline@#! @'' &  (\lstinline@#!@ followed by a space) starts a line comment; see \nameref{subsec:aml-base-lang-reader-comments}. \\
  \lstinline@#!/@ & starts a line comment; see \nameref{subsec:aml-base-lang-reader-comments}. \\
...
A sequence of source characters that starts with ``\lstinline!|!'' is parsed as a (verbatim) symbol until a matching ``\lstinline!|!'' is found. If a ``\lstinline!|!'' should appear inside the symbol, it has to be preceded by a ``\lstinline!\!'', and that character is the only character omitted from the resulting symbol.
...
\begin{tabular}{ l l }
  ...
  \lstinline!~! & adds \lstinline!'unquote! only within a quasiquote \\
  \lstinline!~@! & adds \lstinline!'unquote-splicing! only within a quasiquote \\
  \lstinline!#'! & adds \lstinline!'syntax-quote! \\
  \lstinline!#`! & adds \lstinline!'syntax-quasiquote! \\
  \lstinline!#~! & adds \lstinline!'syntax-unquote! only within a quasiquote \\
  \lstinline!#~@! & adds \lstinline!'syntax-unquote-splicing! only within a quasiquote \\
\end{tabular}
...
\begin{tabular}{ r l }
  ...
  \lstinline!'(apple)! & reads equal to \lstinline!(list 'quote (list 'apple))! \\
...
Outside of a quasiquoted form, all unquote-starting forms are read as-is, as symbols. Inside of a quasiquoted form, a prefix of \lstinline!\! can be used to prevent unquote forms from being read as unquote forms, e.g. ``\lstinline!\~!'' to be read as ``\lstinline!~!'' and whatever consequences that read has (usually continues as \nameref{subsec:aml-base-lang-reader-symbols}). 
